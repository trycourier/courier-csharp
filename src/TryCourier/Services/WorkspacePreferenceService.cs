...
public sealed class WorkspacePreferenceService : IWorkspacePreferenceService
{
    ...
    /// <inheritdoc/>
    public async Task<PreferenceLogsListResponse> ListLogs(
        WorkspacePreferenceListLogsParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        using var response = await this
            .WithRawResponse.ListLogs(parameters, cancellationToken)
            .ConfigureAwait(false);
        return await response.Deserialize(cancellationToken).ConfigureAwait(false);
    }
    ...
    : IWorkspacePreferenceServiceWithRawResponse
{
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<PreferenceLogsListResponse>> ListLogs(
        WorkspacePreferenceListLogsParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        HttpRequest<WorkspacePreferenceListLogsParams> request = new()
        {
            Method = HttpMethod.Get,
            Params = parameters,
        };
        ...
        return new(
            ...
            async (token) =>
            {
                var preferenceLogsListResponse = await response
                    .Deserialize<PreferenceLogsListResponse>(token)
                    .ConfigureAwait(false);
                if (this._client.ResponseValidation)
                {
                    preferenceLogsListResponse.Validate();
                }
                return preferenceLogsListResponse;
            }
        );
    }
