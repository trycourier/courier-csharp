...
public sealed class MessageService : IMessageService
{
    ...
    /// <inheritdoc/>
    public async Task<MessageResendResponse> Resend(
        MessageResendParams parameters,
        CancellationToken cancellationToken = default
    )
    {
        using var response = await this
            .WithRawResponse.Resend(parameters, cancellationToken)
            .ConfigureAwait(false);
        return await response.Deserialize(cancellationToken).ConfigureAwait(false);
    }

    /// <inheritdoc/>
    public Task<MessageResendResponse> Resend(
        string messageID,
    ...
    )
    {
        ...
        return this.Resend(parameters with { MessageID = messageID }, cancellationToken);
    }
}
...
public sealed class MessageServiceWithRawResponse : IMessageServiceWithRawResponse
{
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<MessageResendResponse>> Resend(
        MessageResendParams parameters,
        CancellationToken cancellationToken = default
    )
    {
        ...
        HttpRequest<MessageResendParams> request = new()
        {
            Method = HttpMethod.Post,
            Params = parameters,
        };
        ...
        return new(
            ...
            {
                var deserializedResponse = await response
                    .Deserialize<MessageResendResponse>(token)
                    .ConfigureAwait(false);
                if (this._client.ResponseValidation)
                {
                    deserializedResponse.Validate();
    ...
    /// <inheritdoc/>
    public Task<HttpResponse<MessageResendResponse>> Resend(
        string messageID,
        MessageResendParams? parameters = null,
        CancellationToken cancellationToken = default
    )
