...
public sealed record class TopicPreference : JsonModel
{
    ...
    /// applies. Ids come from the topic's digest configuration, returned on any
    /// read of the topic.
    ///
    /// <para>Also omitted when the schedule they chose has since been deleted or
    /// disabled: the recipient is on the topic's default from that point on, which
    /// is how a send already resolves it, so the field reports what would actually
    /// be used rather than a dead id.</para>
    /// </summary>
