...
///
/// <para>Updating by `schedule_id` replaces that schedule rather than merging into
/// it: any field you leave out is cleared. Two of those change delivery silently
/// — an omitted `timezone` reverts the schedule to UTC, and an omitted `is_default`
/// can leave the topic with no default schedule, which is what recipients who have
/// not chosen one fall back to. Restate every field you want to keep.</para>
/// </summary>
[JsonConverter(
