...
/// run). Supplying both or neither is a `400`. A `run_id` that does not exist for
/// the caller's tenant returns `404`. Cancelation is idempotent and non-clobbering:
/// a run that has already finished (`PROCESSED`/`ERROR`) or was already `CANCELED`
/// is left untouched and its current status is echoed back.
///
