...
/// run is marked `ERROR`, no digest instance is created, and the journey does not
/// continue past this node. Configure the topic's template before using the topic
/// in a journey.</para>
///
/// <para>If the journey run is scoped to a tenant, digests are kept separate per
/// tenant: two runs for the same user under different tenants accumulate and release
/// as separate digests, even on the same topic.</para>
/// </summary>
