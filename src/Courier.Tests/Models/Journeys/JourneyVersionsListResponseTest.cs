using System.Collections.Generic;
using System.Text.Json;
using Courier.Core;
using Courier.Models;
using Courier.Models.Journeys;

namespace Courier.Tests.Models.Journeys;

public class JourneyVersionsListResponseTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var model = new JourneyVersionsListResponse
        {
            Paging = new() { More = true, Cursor = "cursor" },
            Results =
            [
                new()
                {
                    Created = 0,
                    Creator = "creator",
                    Name = "name",
                    Published = 0,
                    Version = "version",
                },
            ],
        };

        Paging expectedPaging = new() { More = true, Cursor = "cursor" };
        List<JourneyVersionItem> expectedResults =
        [
            new()
            {
                Created = 0,
                Creator = "creator",
                Name = "name",
                Published = 0,
                Version = "version",
            },
        ];

        Assert.Equal(expectedPaging, model.Paging);
        Assert.Equal(expectedResults.Count, model.Results.Count);
        for (int i = 0; i < expectedResults.Count; i++)
        {
            Assert.Equal(expectedResults[i], model.Results[i]);
        }
    }

    [Fact]
    public void SerializationRoundtrip_Works()
    {
        var model = new JourneyVersionsListResponse
        {
            Paging = new() { More = true, Cursor = "cursor" },
            Results =
            [
                new()
                {
                    Created = 0,
                    Creator = "creator",
                    Name = "name",
                    Published = 0,
                    Version = "version",
                },
            ],
        };

        string json = JsonSerializer.Serialize(model, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<JourneyVersionsListResponse>(
            json,
            ModelBase.SerializerOptions
        );

        Assert.Equal(model, deserialized);
    }

    [Fact]
    public void FieldRoundtripThroughSerialization_Works()
    {
        var model = new JourneyVersionsListResponse
        {
            Paging = new() { More = true, Cursor = "cursor" },
            Results =
            [
                new()
                {
                    Created = 0,
                    Creator = "creator",
                    Name = "name",
                    Published = 0,
                    Version = "version",
                },
            ],
        };

        string element = JsonSerializer.Serialize(model, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<JourneyVersionsListResponse>(
            element,
            ModelBase.SerializerOptions
        );
        Assert.NotNull(deserialized);

        Paging expectedPaging = new() { More = true, Cursor = "cursor" };
        List<JourneyVersionItem> expectedResults =
        [
            new()
            {
                Created = 0,
                Creator = "creator",
                Name = "name",
                Published = 0,
                Version = "version",
            },
        ];

        Assert.Equal(expectedPaging, deserialized.Paging);
        Assert.Equal(expectedResults.Count, deserialized.Results.Count);
        for (int i = 0; i < expectedResults.Count; i++)
        {
            Assert.Equal(expectedResults[i], deserialized.Results[i]);
        }
    }

    [Fact]
    public void Validation_Works()
    {
        var model = new JourneyVersionsListResponse
        {
            Paging = new() { More = true, Cursor = "cursor" },
            Results =
            [
                new()
                {
                    Created = 0,
                    Creator = "creator",
                    Name = "name",
                    Published = 0,
                    Version = "version",
                },
            ],
        };

        model.Validate();
    }

    [Fact]
    public void CopyConstructor_Works()
    {
        var model = new JourneyVersionsListResponse
        {
            Paging = new() { More = true, Cursor = "cursor" },
            Results =
            [
                new()
                {
                    Created = 0,
                    Creator = "creator",
                    Name = "name",
                    Published = 0,
                    Version = "version",
                },
            ],
        };

        JourneyVersionsListResponse copied = new(model);

        Assert.Equal(model, copied);
    }
}
